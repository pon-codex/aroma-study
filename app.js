...
function renderCards(){let list=oils.filter(o=>(filter==='すべて'||o.cat===filter)&&(Object.values(o).join(' ')+' '+aromaDescriptions[o.name]+' '+bodyConcerns[o.name]+' '+plantFamilies[o.name]).toLowerCase().includes(query.toLowerCase()));$('#resultCount').textContent=`${list.length}種`;$('#cardGrid').innerHTML=list.map(o=>{const [symptoms,detail]=bodyConcerns[o.name].split('｜');return `<article class="oil-card ${flipped.has(o.name)?'flipped':''}" data-name="${o.name}"><div class="card-inner"><div class="face front"><span class="oil-no">${String(oils.indexOf(o)+1).padStart(2,'0')} / ESSENTIAL OIL</span><span class="level-badge">${o.level===2?'2級対象':'1級追加'}</span><h3>${o.name}</h3><span class="latin">${o.latin}</span><span class="family"><small>植物科名</small>${plantFamilies[o.name]}</span><div class="aroma-profile"><span class="scent">${o.scent}香り</span><p>${aromaDescriptions[o.name]}</p></div><span class="tag">${o.cat}　·　${o.note}</span></div><div class="face back"><h4>${o.name}｜身体の不調と活用</h4><p><b>科名：</b>${plantFamilies[o.name]}　<b>主な成分：</b>${o.key}</p><div class="body-box"><b>選ばれることがある不調</b><p>${symptoms}</p></div><p class="body-detail">${detail}</p><div class="effect-list compact">${o.effect.split('｜').filter(x=>!x.startsWith('身体：')).map(x=>`<p>${x}</p>`).join('')}</div><span class="mark">※診断・治療の代わりではありません</span></div></div></article>`}).join('');document.querySelectorAll('.oil-card').forEach(c=>c.onclick=()=>{flipped.has(c.dataset.name)?flipped.delete(c.dataset.name):flipped.add(c.dataset.name);renderCards()})}
